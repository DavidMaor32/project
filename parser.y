...
func_ret: type func_sign BLOCK_OPEN ret_func return BLOCK_CLOSE
    | type func_sign BLOCK_OPEN ret_func {yyerror("missing return statement!\n");} BLOCK_CLOSE ;

func_void: VOID { flag=1; } func_sign structured_b;
...
return: RETURN expr SEMICOL {if (flag==1)yyerror("void function can't return!\n");} 

/* vvvvvvvvvvvvvv DONT - TOUCH  vvvvvvvvvvvvvv */
/* ============== BODY - BLOCK  ============== */
ret_body: structured_b | ret_stmt ;
structured_b: BLOCK_OPEN ret_block BLOCK_CLOSE ;
ret_func:  return block | block ;
block : dec stmts | dec | stmts ;
ret_block : dec ret_stmts | dec | ret_stmts | ;
...
if_stmt: IF PARENT_OPEN expr PARENT_CLOSE ret_body %prec IFX;

if_else_stmt: IF PARENT_OPEN expr PARENT_CLOSE ret_body ELSE ret_body;

loop_stmt: for | while | do;

for: FOR PARENT_OPEN init SEMICOL expr SEMICOL ID ASS expr PARENT_CLOSE ret_body;
init: ID ASS expr;

do: DO ret_body WHILE PARENT_OPEN expr PARENT_CLOSE SEMICOL;
while: WHILE PARENT_OPEN expr PARENT_CLOSE ret_body;
